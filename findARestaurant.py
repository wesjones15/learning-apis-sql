...
google_api_key = lines[0].rstrip()
foursquare_client_id = lines[1].rstrip()
foursquare_client_secret = lines[2].rstrip()
...
def findARestaurant(mealType, location):
    ...
    url = ('https://api.foursquare.com/v2/venues/search?client_id=%s&client_secret=%s&v=20190425&ll=%s,%s&query=%s' % (foursquare_client_id, foursquare_client_secret,latitude,longitude,mealType))
    h = httplib2.Http()
    result = json.loads(h.request(url,'GET')[1])
    if result['response']['venues']:
        ...
        url = ('https://api.foursquare.com/v2/venues/%s/photos?client_id=%s&v=20190425&client_secret=%s' % ((venue_id,foursquare_client_id,foursquare_client_secret)))
        result = json.loads(h.request(url,'GET')[1])
...
if __name__ == '__main__':
    ...
    mealType = input("What type of food do you want? ")
